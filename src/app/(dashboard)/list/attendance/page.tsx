...
const AttendancePage = () => {
  ...
  const fetchStudents = async () => {
    setLoading(true);
    try {
      const res = await apiClient.get(`/attendance?page=${page}&limit=10&search=${encodeURIComponent(search)}`);
      if (res.status === 200) {
    ...
    <tr key={student.id} className="even:bg-primary-light text-sm">
      ...
      <td>
        <input
          ...
          onChange={() => !attendanceMarked && handleCheck(student.id, "PRESENT")}
          disabled={attendanceMarked || attendance[student.id] === "ABSENT"}
        />
      </td>
      <td>
        <input
          ...
          onChange={() => !attendanceMarked && handleCheck(student.id, "ABSENT")}
          disabled={attendanceMarked || attendance[student.id] === "PRESENT"}
        />
      </td>
    </tr>
  );
  ...
  return (
    <div className="bg-primary-light p-4 rounded-md flex-1 m-4 mt-0">
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold">Take Attendance</h1>
